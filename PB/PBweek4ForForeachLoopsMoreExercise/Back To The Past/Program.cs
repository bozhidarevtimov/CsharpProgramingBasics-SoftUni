...
namespace Back_To_The_Past
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //променливи
            //годините на Иван в началото = 18
            int IvanAge = 18;
            //годината в която се връща = 1800
            int yearInThePast = 1800;
            //пари които ще му трябват
            double moneyNeeded = 0;

            //2. Цикъл който да се върти от годината в която се е върнал до годината до която трябва да живее.
            for (int i = yearInThePast; i <= yearToLive; i++)
            {
                // Като приемем, че за всяка четна(1800, 1802 и т.н.) година ще харчи 12 000 лева.
                // За всяка нечетна(1801, 1803  и т.н.) ще харчи 12 000 + 50 * [годините, които е навършил през дадената година].
                //Да изчислим дали ще му стигнат парите
                if (i%2==0)
                {
                    moneyNeeded += 12000;
                }
            ...
            if (heritageMoney>=moneyNeeded)
            {
                Console.WriteLine($"Yes! He will live a carefree life and will have {heritageMoney-moneyNeeded:f2} dollars left.");
            }
            else
            {
                Console.WriteLine($"He will need {moneyNeeded-heritageMoney:f2} dollars to survive.");
            }
        }
    }
}
